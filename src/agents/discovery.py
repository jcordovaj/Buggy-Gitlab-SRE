...
# Caché global en memoria indexado por project_id para optimizar operaciones
_DISCOVERY_CACHE: Dict[str, RepositoryProfile] = {}
...
class RepositoryAnalyzer:
    ...
    async def analyze_repo(self, project_id: str) -> RepositoryProfile:
        ...
        utilizando la nueva herramienta MCP de listado plano de estructura.
        """
        # 1. Verificación en Caché Operacional
        if project_id in _DISCOVERY_CACHE:
            logger.info(f"[💾] Perfil de repositorio recuperado de caché para Proyecto {project_id}")
            return _DISCOVERY_CACHE[project_id]

        logger.info(f"[🔍] Iniciando descubrimiento agnóstico para el repositorio ID: {project_id}")
        
        # 2. Consumo de la herramienta MCP barata y correcta de estructura plana
        ...
        if not response.success or not response.data:
            ...
            return self._generate_unknown_profile("Fallo en la consulta de estructura MCP.")

        file_list: List[str] = response.data.get("files", [])
        
        # 3. Fase Determinista: Puntuación de Evidencia Física Real
        profile = await self._evaluate_deterministically(file_list)
        
        # 4. Fallback Cognitivo Acotado: Solo si la evidencia es ambigua (Confidence < 0.95)
        if profile.confidence_score < 0.95:
            ...
            profile = await self._call_cognitive_resolution(file_list, profile)

        # 5. Guardar en caché antes de retornar
        _DISCOVERY_CACHE[project_id] = profile
        return profile

    async def _evaluate_deterministically(self, files: List[str]) -> RepositoryProfile:
        ...
        if "pom.xml" in files:
            ...
            # Inspección quirúrgica posterior refinando el framework sin asumir
        ...
        # --- ECOSISTEMA PYTHON ---
            ...
            lang, build, runner, score = "python", "pip", "pytest", score + 0.85
            evidence.append("✓ Manifiesto de entorno Python")
            
            # Verificación dirigida para el framework
        ...
        return RepositoryProfile(
            language=lang, framework=framework, build_system=build,
            test_runner=runner, ci_pipeline=ci,
            confidence_score=min(score, 1.0), evidence=evidence
        )
    ...
    def _generate_unknown_profile(self, reason: str) -> RepositoryProfile:
        return RepositoryProfile(
            language="unknown", framework="unknown", build_system="unknown",
            test_runner="unknown", ci_pipeline="unknown", confidence_score=0.0,
            evidence=[f"Fallo en descubrimiento estático: {reason}"]
        )

    async def _call_cognitive_resolution(self, files: List[str], partial_profile: RepositoryProfile) -> RepositoryProfile:
        ...
        # Reducimos drásticamente los tokens enviando solo la lista de strings del árbol
        prompt = f"""
        ...
        try:
            response = self._gemini_client.models.generate_content(
                ...
                config=self._types.GenerateContentConfig(
                    system_instruction="Eres un Discovery Agent experto. Devuelve obligatoriamente la estructura RepositoryProfile.",
